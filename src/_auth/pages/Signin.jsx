...
import { capitalizeFirstLetter, setLocalStorageItem, generateRandomPassword } from "@/lib/utils";
import { signInWithGithub, signInWithGoogle, getCurrentAccount, signOutAccount } from "@/lib/appwrite/api";
import { useCreateUserAccount, useLoginUserAccount, useLoginUserAccountWithSocial } from "@/lib/react-query";
...
export const Signin = () => {
  ...
  const onSubmit = async data => {
    // Allow user to login with email or username both
  ...
  useEffect(() => {
    ...
    const authenticateUser = async () => {
      ...
      if (!account?.user || !account?.session) {
        setIsLoading(false);
        return showAlert("Sign in Failed.", "Something went wrong.", "error");
      }
      ...
      const userData = {
        name: user?.name,
        username: user?.name.toLowerCase().replace(" ", "") + `-${Date.now()}`,
        email: user?.email,
      ...
      const res = await loginWithSocial(userData);
  ...
  return (
    <section className="w-full h-screen flex flex-col items-center justify-center sm:px-4">
      <Helmet>
        <title>Signin - Linkbridge</title>
      </Helmet>
      
      <div className="w-full space-y-6 text-gray-600 sm:max-w-md">
        <div className="text-center">
          <div className="mt-5 space-y-2">
            <h3 className="text-gray-800 text-2xl font-bold sm:text-3xl dark-text1">Log in to your account</h3>
            <p className="dark-text1">
              Don't have an account?{" "}
              <Link to="/sign-up" className="font-medium text-purple-80 hover:text-purple-70">
                Sign up
              </Link>
            </p>
          </div>
        </div>
        <div className="bg-white rounded-md shadow p-4 py-6 space-y-8 sm:p-6 sm:rounded-lg dark-mode">
          <div className="grid grid-cols-2 gap-x-3">
            <button
              disabled={isLoginingWithSocial || isLogining}
            ...
            <button
              ...
              className="flex items-center justify-center py-2.5 border rounded-lg hover:bg-gray-50 duration-150 active:bg-gray-100 dark:bg-white "
            >
              <svg className="w-5 h-5" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="relative">
            <span className="block w-full h-px bg-gray-300 dark:bg-gray-60"></span>
            <p className="inline-block w-fit text-sm bg-white px-2 absolute -top-2 inset-x-0 mx-auto dark:bg-dark-secondary dark:text-gray-60">
              Or continue with
            </p>
          </div>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div>
              ...
              <input type="text" className="auth-form-input" {...register("email")} />
              <p className="error-message">{errors?.email && capitalizeFirstLetter(errors.email?.message)}</p>
            </div>
            <div>
              <label className="font-medium">Password</label>
              <input type="password" required className="auth-form-input" {...register("password")} />
              <p className="error-message">{errors?.password && capitalizeFirstLetter(errors.password?.message)}</p>
            </div>
            <button
              disabled={isLoginingWithSocial || isLogining}
              className="w-full px-4 py-2 text-white font-medium bg-black/90 hover:bg-black/95 rounded-lg duration-150 disabled:bg-black/70 dark:bg-white dark:text-black"
            >
              {isLoginingWithSocial || isLogining ? <Loading /> : "Sign in"}
            </button>
          </form>
        </div>
        <div className="text-center dark-text2">
